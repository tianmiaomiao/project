...
```javascript
import Dialog from  '../../js/components/modules/dialog'
...
| align | 对齐方式 | String | `center`| `right / left / center`|
| closable | 是否显示清空按钮 | Boolean | `true`|-|
| btns | 底部操作按钮组 | Array | `[]`|-|
| append-to | 组件的挂载节点 | HTMLElement | `document.body`|-|
| has-mask | 是否有蒙层 | Boolean | `true`|-|
| mask-closable | 点击蒙层是否可关闭弹出层 | Boolean | `false`|-|
| position | 弹出层位置, `center/top/bottom` | String | `center`|-|
| transition | 弹出层过度动画, `fade, slide-up/slide-down/`  | String | `fade`|-|
